...
An important feature to consider when buying a new car is the transmission type, as it impacts your driving experience, affordability, and fuel efficiency. Direct drive transmissions is fuel efficient only when driving on a flat ground highway. Manual transmissions provide a more engaging driving experience and saves you money on fuel and maintanence. While automatic transmissions are more popular due to eliminating the need to change gears manually, they are typically less fuel efficient. (https://www.dennisdillonmazda.com/which-transmission-type-is-right-for-you-boise-id/#:~:text=Equipping%20yourself%20with%20the%20necessary,make%20a%20well%2Dinformed%20decision.) An automated manual transmission combines both transmission features, offering fuel efficiency from automatic and the cost-effectiveness of a manual transmission. Out of the transmissions, direct drive is the most expensive, followed by automatic transmission, and last manual and automatic manual transmissions. (https://www.autotransrus.com.au/blog/amt/#:~:text=AMT%20offers%20a%20compromise%20between,ease%20of%20use%20is%20compromised.)
(https://www.truckinginfo.com/155143/direct-drive-or-overdrive#:~:text=It%20all%20depends%20on%20the,overdrive%20becomes%20a%20better%20fit.)
...
MSRP_outliers <- ggplot(NA_car_data, aes(MSRP)) +
  geom_boxplot(fill = "plum") + labs(x = "MSRP (in thousands)")
...
MSRP_noOutliers <- ggplot(outliers_removed_car_NA, aes(MSRP)) +
  geom_boxplot(fill = "plum3") + labs(x = "MSRP (in thousands)")
...
Now, we have the significant predictors based on the backward selection. It is time to check the VIF, Variance Inflation Factor. The VIF function measures the amount of multicollinearity in the regression equation. Multicollinearity is when two independent variables are correlated with each other. As you can see, there are some variables missing from the backward selection variables and the model_NA variable. I removed Model for its high quantity of categories which would increase the complexity of the stepwise model. Additionally, I removed `highway.MPG` because it was correlated with `city.mpg`
between the two `highway.MPG` was more strongly connected to other variables.
...
“Understanding MSRP: A Guide to Manufacturer Pricing | Indeed.Com.” Indeed, www.indeed.com/career-advice/career-development/msrp. Accessed 03 Dec. 2024.
