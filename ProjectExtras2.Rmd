...
top_10_pcs <- loadings[, 1:10]
pca_loadings_df <- as.data.frame(top_10_pcs)
pca_loadings_df$Feature <- rownames(loadings)

tidy_loadings <- pivot_longer(pca_loadings_df, cols = starts_with("PC"),
                              names_to = "Principal_Component",
                              values_to = "Loading")

ggplot(tidy_loadings, aes(x = reorder(Feature, abs(Loading)), y = abs(Loading),
                          fill = Principal_Component)) +
  geom_bar(stat = "identity") +
  facet_wrap(~Principal_Component, scales = "free") + coord_flip() 
...
heatmap_data <- as.data.frame(pca_results_2$rotation[, 1:10])  # First 10 PCs
heatmap_data$Feature <- rownames(pca_results_2$rotation)
heatmap_data <- melt(heatmap_data, id.vars = "Feature", variable.name = "PC", value.name = "Loading")


ggplot(heatmap_data, aes(x = PC, y = Feature, fill = Loading)) +
  geom_tile() +
  scale_fill_gradient2(low = "blue", high = "red", mid = "white", midpoint = 0) +
  labs(title = "Feature Contributions to Principal Components", x = "Principal Component", y = "Feature") +
  theme_minimal()
```

```{r}
pca_recipe_NA1 <- recipe(MSRP ~ ., data = NA_car_training) %>% 
  step_dummy(all_nominal_predictors()) %>% 
  step_zv(all_predictors()) %>% 
  step_center(all_predictors()) %>% 
  step_scale(all_predictors()) %>% 
...
pca_model_NA1 <- linear_reg() %>% 
  set_engine("lm") %>% 
  set_mode("regression")

pca_workflow_NA1 <- workflow() %>% 
  add_model(pca_model_NA1) %>% 
  add_recipe(pca_recipe_NA1) 

pca_fit_NA1 <- fit_resamples(pca_workflow_NA1, NA_car_folds)

pca_metrics_NA1 <- collect_metrics(pca_fit_NA1) %>% 
  filter(.metric == "rmse") %>% 
  arrange(mean) %>% 
  slice_head(n=1)
...
car_data %>% 
  head()
```
The `head()` function prints the first six rows of the dataset. Most of the columns represent various car features, such as make, model, year, and engine fuel type. One column represents the MSRP.
